...
const rebootReasonArray = [
  "RESTART_AFTER_POWER_FAILIURE",
  "BLUETOOTH_COMMAND",
  "LORAWAN_COMMAND",
  "POWER_ON",
];
...
function consume(event) {
  ...
  if (port <= 10) {
    ...
    if (port === 2 || port === 3) {
      lifecycle.positioningType =
        bytes[0] & (0x40 === 0) ? "NORMAL" : "DOWNLINK_FOR_POSITION";
    }
  ...
  if (port === 1) {
    ...
    emit("sample", {
      data: {
        shutdownType:
          shutdownTypeArray[Bits.bitsToUnsigned(bits.substr(24, 8))],
      },
      topic: "shutdown",
    });
  } else if (port === 5) {
    emit("sample", {
      data: { numberOfShocks: Bits.bitsToUnsigned(bits.substr(24, 16)) },
      topic: "vibration",
    });
  } else if (port === 6) {
    emit("sample", {
      data: { totalIdleTime: Bits.bitsToUnsigned(bits.substr(24, 16)) },
      topic: "mandown",
    });
  } else if (port === 7) {
    let parseLen = 3; // common head is 3 byte
    ...
    if (
      movement.eventType === "START_OF_MOVEMENT" ||
      movement.eventType === "IN_MOVEMENT"
    ) {
