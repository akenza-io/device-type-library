...
function consume(event) {
  ...
  if (header == 76 || header == 108) { // Learning
    topic = "learning";
  ...
  } else if (header == 82 || header == 114) { // Report
    topic = "report";

    data.anomalyLevel = round(Bits.bitsToUnsigned(bits.substr(8, 8)) * 100 / 127);
    ...
    data.operatingTime = (Bits.bitsToUnsigned(bits.substr(16, 8))) * reportLength / 127;
    ...
    data.min0_10 = round(Bits.bitsToUnsigned(bits.substr(24, 8)) * data.operatingTime / 127);
    data.min10_20 = round(Bits.bitsToUnsigned(bits.substr(96, 8)) * (data.operatingTime - data.min0_10) / 127);
    data.min20_40 = round(Bits.bitsToUnsigned(bits.substr(104, 8)) * (data.operatingTime - data.min0_10) / 127);
    data.min40_60 = round(Bits.bitsToUnsigned(bits.substr(112, 8)) * (data.operatingTime - data.min0_10) / 127);
  ...
  } else if (header == 65 || header == 97) { // Alarm
    topic = "alarm";
    data.anomalyLevel = round(Bits.bitsToUnsigned(bits.substr(8, 8)) * 100 / 127);
