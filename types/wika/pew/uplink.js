...
const CHANNEL_NAMES_DICTIONARY = [
  "pressure",
  "deviceTemperature",
  "batteryVoltage",
];
...
const TECHNICAL_ALARM_TYPE_NAMES_DICTIONARY = {
  1: "ALU_SATURATION_ALARM",
  2: "SENSOR_MEMORY_INTEGRETY_ERROR",
  4: "SENSOR_BUSY_ERRPR",
  8: "RESERVED",
  16: "SENSOR_COMMUNICATION_ERROR",
  32: "PRESSURE_OUT_OF_LIMIT",
  64: "TEMPERATURE_OUT_OF_LIMIT",
};
/**
 * @type {{[key: number]: string}}
 */
const DEVICE_ALARM_TYPE_NAMES_DICTIONARY = {
  0: "LOW_BATTERY",
  4: "ACKNOWLEDGE_MESSAGE_NOT_EMITTED",
};
const DEVICE_ALARM_CAUSE_OF_FAILURE_NAMES_DICTIONARY = [
  "GENERIC",
  "DEVICE_DEPENDENT",
];
  ...
  switch (input.bytes[0]) {
    ...
    case 0x02:
      ...
      if (input.bytes.length == 7) {
        ...
        output = addErrorMessage(
          output,
          `Data message 01/02 needs 7 bytes but got ${input.bytes.length}`,
        );
      }
      break;
    ...
    case 0x03:
      ...
      if (input.bytes.length >= 5 && !((input.bytes.length - 2) % 3)) {
        ...
        output = addErrorMessage(
          output,
          `Process alarm 03 needs at least 5 bytes and got ${input.bytes.length}. Also all bytes for each alarm needed`,
        );
      }
      break;
    ...
    case 0x04:
      ...
      if (input.bytes.length == 3) {
        ...
        output = addErrorMessage(
          output,
          `Technical alarm 04 needs 5 bytes but got ${input.bytes.length}. Also all bytes for each alarm needed`,
        );
      }
      break;
    ...
    case 0x05:
      ...
      if (input.bytes.length >= 3 && input.bytes.length <= 4) {
        ...
        output = addErrorMessage(
          output,
          `Device alarm 05 needs 4 bytes but got ${input.bytes.length}`,
        );
      }
      break;
    ...
    case 0x07:
      ...
      if (input.bytes.length >= 8 && input.bytes.length <= 38) {
        ...
        output = addErrorMessage(
          output,
          `Identification message 07 needs 41 bytes but got ${input.bytes.length}`,
        );
      }
      break;
...
function decodeDataMessage(input) {
  ...
  pressure.value = getCalculatedPressure(
  ...
  temperature.value = getCalculatedTemperature(
    (input.bytes[5] << 8) | input.bytes[6],
  );
  ...
  batteryVoltage.channelName =
    CHANNEL_NAMES_DICTIONARY[batteryVoltage.channelId];
...
function decodeProcessAlarm(input) {
  ...
  for (
    let byteIndex = 2, alarmCounter = 0;
    ...
    output.data.processAlarms[alarmCounter].channelId =
      (input.bytes[byteIndex] & 0x78) >> 3;
    output.data.processAlarms[alarmCounter].channelName =
      ALARM_CHANNEL_NAMES_DICTIONARY[
        output.data.processAlarms[alarmCounter].channelId
      ];
    ...
    // Alarm value
    output.data.processAlarms[alarmCounter].value =
      getRealValueByChannelNumberAndAlarmType(
        output.data.processAlarms[alarmCounter].channelId,
        output.data.processAlarms[alarmCounter].alarmType,
    ...
    if (output.data.processAlarms[alarmCounter].value == ERROR_VALUE) {
      output = addWarningMessage(
        output,
        `Invalid data for ${output.data.processAlarms[alarmCounter].channelName}channel`,
      );
    }
  }
...
function decodeTechnicalAlarm(input) {
  ...
  for (
    ...
    // Alarm event 0 = triggered, 1 = disappeared
    output.data.technicalAlarms[alarmCounter].event =
      (input.bytes[byteIndex] & 0x80) >> 7;
    output.data.technicalAlarms[alarmCounter].eventName =
      ALARM_EVENT_NAMES_DICTIONARY[
        output.data.technicalAlarms[alarmCounter].event
      ];
...
function decodeDeviceAlarm(input) {
  ...
  output.data.deviceAlarm.eventName =
    ALARM_EVENT_NAMES_DICTIONARY[output.data.deviceAlarm.event];

  // Generic or device dependent 1 = device dependent
  output.data.deviceAlarm.causeOfFailure = (input.bytes[2] & 0x60) >> 6;
  output.data.deviceAlarm.causeOfFailureName =
    DEVICE_ALARM_CAUSE_OF_FAILURE_NAMES_DICTIONARY[
      output.data.deviceAlarm.causeOfFailure
    ];

  // Alarm type 0 = low battery, 4 = Acknowledged message not emitted
  output.data.deviceAlarm.alarmType = input.bytes[2] & 0x1f;
  output.data.deviceAlarm.alarmTypeName =
    DEVICE_ALARM_TYPE_NAMES_DICTIONARY[output.data.deviceAlarm.alarmType];
...
function decodeDeviceIdentification(input) {
  ...
  output.data.deviceInformation.productIdName =
    input.bytes[2] == 11 ? "PEW" : input.bytes[2];
  ...
  if (input.bytes.length < 38) {
    output = addErrorMessage(
      output,
      `Device identification frame 07 has not all bytes included, received ${input.bytes.length}/38 bytes`,
    );
    return output;
  }
  ...
  // Min range pressure
  output.data.deviceInformation.measurementRangeStartPressure =
  ...
  output.data.deviceInformation.measurementRangeStartPressure = Number(
    output.data.deviceInformation.measurementRangeStartPressure.toFixed(6),
  );

  // Max range pressure
  output.data.deviceInformation.measurementRangeEndPressure =
    convertHexToFloatIEEE754(
      input.bytes[24].toString(16).padStart(2, "0") +
        input.bytes[25].toString(16).padStart(2, "0") +
        input.bytes[26].toString(16).padStart(2, "0") +
        input.bytes[27].toString(16).padStart(2, "0"),
    );
  output.data.deviceInformation.measurementRangeEndPressure = Number(
    output.data.deviceInformation.measurementRangeEndPressure.toFixed(6),
  );

  // Min range device temperature
  output.data.deviceInformation.measurementRangeStartDeviceTemperature =
    convertHexToFloatIEEE754(
      input.bytes[28].toString(16).padStart(2, "0") +
        input.bytes[29].toString(16).padStart(2, "0") +
        input.bytes[30].toString(16).padStart(2, "0") +
        input.bytes[31].toString(16).padStart(2, "0"),
    );
  output.data.deviceInformation.measurementRangeStartDeviceTemperature = Number(
    output.data.deviceInformation.measurementRangeStartDeviceTemperature.toFixed(
      6,
    ),
  );

  // Max range device temperature
  output.data.deviceInformation.measurementRangeEndDeviceTemperature =
    convertHexToFloatIEEE754(
      input.bytes[32].toString(16).padStart(2, "0") +
        input.bytes[33].toString(16).padStart(2, "0") +
        input.bytes[34].toString(16).padStart(2, "0") +
        input.bytes[35].toString(16).padStart(2, "0"),
    );
  output.data.deviceInformation.measurementRangeEndDeviceTemperature = Number(
    output.data.deviceInformation.measurementRangeEndDeviceTemperature.toFixed(
      6,
    ),
  );
  ...
  output.data.deviceInformation.pressureUnitName = returnPhysicalUnitFromId(
    input.bytes[36],
  );

  // Unit pressure
  output.data.deviceInformation.deviceTemperatureUnit = input.bytes[37];
  output.data.deviceInformation.deviceTemperatureUnitName =
    returnPhysicalUnitFromId(input.bytes[37]);
...
function checkMeasurementRanges(output) {
  ...
  if (typeof DEVICE_TEMPERATURE_RANGE_START === "undefined") {
    output = addErrorMessage(
      output,
      "The DEVICE_TEMPERATURE_RANGE_START was not set.",
    );
  }
  ...
  if (typeof DEVICE_TEMPERATURE_RANGE_END === "undefined") {
    output = addErrorMessage(
      output,
      "The DEVICE_TEMPERATURE_RANGE_END was not set.",
    );
  }
  ...
  if (PRESSURE_RANGE_START >= PRESSURE_RANGE_END) {
    output = addErrorMessage(
      output,
      `The PRESSURE_RANGE_START must not be greater or equal to PRESSURE_RANGE_END, ${PRESSURE_RANGE_START} >= ${PRESSURE_RANGE_END}. `,
    );
  }
  ...
  if (DEVICE_TEMPERATURE_RANGE_START >= DEVICE_TEMPERATURE_RANGE_END) {
    output = addErrorMessage(
      output,
      `The DEVICE_TEMPERATURE_RANGE_START must not be greater or equal to DEVICE_TEMPERATURE_RANGE_END, ${DEVICE_TEMPERATURE_RANGE_START} >= ${DEVICE_TEMPERATURE_RANGE_END}.`,
    );
  }
...
function getCalculatedTemperature(channelValue) {
  return getCalculatedValue(
    channelValue,
    DEVICE_TEMPERATURE_RANGE_START,
 ...
 * @access private
 */
function getRealValueByChannelNumberAndAlarmType(
  channelNumber,
  alarmType,
  channelValue,
) {
...
function consume(event) {
  ...
  if (event.device !== undefined && event.device.customFields !== undefined) {
    ...
    if (
      !isNaN(customFields.pressureRangeStart) &&
      customFields.pressureRangeStart !== null &&
      !isNaN(customFields.pressureRangeEnd) &&
      customFields.pressureRangeEnd !== null &&
      !isNaN(customFields.temperatureRangeStart) &&
      customFields.temperatureRangeStart !== null &&
      !isNaN(customFields.temperatureRangeEnd) &&
      customFields.temperatureRangeEnd !== null
    ) {
      ...
      if (decoded.errors !== undefined) {
        ...
        if (
          data.deviceTemperature !== undefined &&
        ...
        data.pressureRangeStart =
          decoded.deviceInformation.measurementRangeStartPressure;
        data.pressureRangeEnd =
          decoded.deviceInformation.measurementRangeEndPressure;
        data.temperatureRangeStart =
          decoded.deviceInformation.measurementRangeStartDeviceTemperature;
        data.temperatureRangeEnd =
          decoded.deviceInformation.measurementRangeEndDeviceTemperature;
        data.pressureUnit = decoded.deviceInformation.pressureUnitName;
        data.deviceTemperatureUnit =
          decoded.deviceInformation.deviceTemperatureUnitName;
