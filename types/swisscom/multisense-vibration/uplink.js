...
function consume(event) {
  ...
  if (port == 3) {
    ...
    while (pointer !== bits.length) {
      ...
      var dataId = Bits.bitsToUnsigned(bits.substr(pointer, 8)); pointer += 8;
      switch (dataId) {
        case 1:
          data.temperature = Math.round((Bits.bitsToUnsigned(bits.substr(pointer, 16)) * 0.01) * 100) / 100; pointer += 16;
          break;
        case 2:
          data.humidity = Bits.bitsToUnsigned(bits.substr(pointer, 8)) * 0.5; pointer += 8;
          break;
        case 3:
          data.reedCounter = Bits.bitsToUnsigned(bits.substr(pointer, 16)); pointer += 16;
          break;
        case 4:
          data.motionCounter = Bits.bitsToUnsigned(bits.substr(pointer, 16)); pointer += 16;
          break;
        case 5:
    ...
    if (data.mode == 1) {
      ...
      if (status & 0x80) {
        emit('sample', {
          data: {
            "motionCounter": data.motionCounter,
            "accelerometerX": data.accelerometerX,
            "accelerometerY": data.accelerometerY,
            "accelerometerZ": data.accelerometerZ
          }, topic: "vibration"
        });
        delete data.motionCounter;
        delete data.accelerometerX;
        delete data.accelerometerY;
        delete data.accelerometerZ;
      }
      if (status & 0x40) {
        emit('sample', { data: data, topic: "button_event" });
      }
