...
function consume(event) {
  ...
  for (let i = 0; i < bytes.length; ) {
    ...
    // PIR
      ...
      decoded.pir = readUInt16LE(Array.from(bytes).slice(i, i + 2));
      i += 2;
    }
    // LIGHT
    else if (channelId === 0x06 && channelType === 0x65) {
      decoded.light = readUInt16LE(Array.from(bytes).slice(i, i + 2));
      decoded.visibleInfrared = readUInt16LE(
        Array.from(bytes).slice(i + 2, i + 4),
      );
      decoded.infrared = readUInt16LE(Array.from(bytes).slice(i + 4, i + 6));
      i += 6;
    }
    // CO2
