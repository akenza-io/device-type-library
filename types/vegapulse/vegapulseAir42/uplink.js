...
function consume(event) {
  ...
  if (
    ...
    const baseDistance = baseDistanceCalculation(
      distance.sensorMeasurementUnit,
      bytesToFloat(bytes.slice(2, 6)),
    );
    ...
    (packetIdentifier >= 8 && packetIdentifier <= 15) ||
    (packetIdentifier >= 18 && packetIdentifier <= 22)
  ) {
    ...
    if (
      [8, 9, 10, 11, 12, 13, 14, 15, 18, 19].indexOf(packetIdentifier) !== -1
    ) {
      ...
      distance = Object.assign(
        differentMeasurementUnits(baseDistance),
        distance,
      );
    ...
    if ([12, 13, 14, 15, 22].indexOf(packetIdentifier) !== -1) {
      if (!(bytes[position] === 0x80) && !(bytes[position + 1] === 0x00)) {
        distance.percentValue =
          bytesToInt16(bytes.slice(position, position + 2)) / 100;
      }
      position += 2;

      if (!(bytes[position] === 0x80) && !(bytes[position + 1] === 0x00)) {
        distance.linearPercentValue =
          bytesToInt16(bytes.slice(position, position + 2)) / 100;
      }
      position += 2;
    ...
    if ([9, 11, 13, 15, 21].indexOf(packetIdentifier) !== -1) {
      ...
      if (Number.isNaN(gps.latitude) && (bytes[position] & 0x80) > 0) {
      ...
      if (Number.isNaN(gps.longitude) && (bytes[position] & 0x80) > 0) {
        // NaN with sign bit set = GPS not attempted, otherwise GPS failed
    ...
    if ([10, 11, 14, 15, 20].indexOf(packetIdentifier) !== -1) {
      distance.detailedState = bytesToUInt32(
    ...
    if (
      [8, 9, 10, 11, 12, 13, 14, 15, 18, 20].indexOf(packetIdentifier) !== -1
    ) {
      if (!(bytes[position] === 0x80) && !(bytes[position + 1] === 0x00)) {
    ...
    if ([16, 23].indexOf(packetIdentifier) !== -1) {
      ...
      position += 4;
      system.manufacturerId = bytesToUInt32(
        bytes.slice(position, position + 4),
      );
      position += 4;
    }
    ...
    if ([16, 25].indexOf(packetIdentifier) !== -1) {
      ...
      schedule.changeCounter = bytesToUInt16(
