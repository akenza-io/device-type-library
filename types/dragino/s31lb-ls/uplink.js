...
function decodeSensorPort2(bytes) {
  ...
  emit("sample", {
    data: { alarmFlag, pa8, temperature, temperatureF, humidity, modStatus },
    topic: "default",
    timestamp,
  });
  emit("sample", {
    data: { batteryVoltage, batteryLevel },
    topic: "lifecycle",
  });
}
...
function decodeSensorPort3(bytes) {
  ...
  emit("sample", {
    data: { alarmFlag, pa8, temperature, temperatureF, humidity, pollMessage },
    topic: "default",
    timestamp,
  });
}
...
function decodeSensorPort5(bytes) {
  ...
  emit("sample", {
    data: { sensorModel, firmwareVersion, frequencyBand, subBand },
    topic: "config",
  });
  emit("sample", {
    data: { batteryVoltage, batteryLevel },
