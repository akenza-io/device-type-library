...
function parseDatalog(offset, bytes) {
    ...
    const timestamp = (bytes[offset + 7] << 24) | (bytes[offset + 8] << 16) | (bytes[offset + 9] << 8) | bytes[offset + 10];
...
function consume(event) {
    ...
    if (port === 0x02) {
        dataLifecycle.batteryVoltage = ((bytes[0] << 8 | bytes[1]) & 0x3FFF) / 1000;
        ...
        const pnack = ((bytes[0] >> 7) & 0x01) ? true : false;
        ...
        for (let i = 1; i < bytes.length; i += 11) {
            ...
            // Emit immediately with the historical timestamp
            emit("sample", { data: historyDefault, topic: "default", timestamp: log.date });
            emit("sample", { data: historyLifecycle, topic: "lifecycle", timestamp: log.date });
        }
        ...
        if (bytes[0] === 0x29) dataConfiguration.sensorModel = "DDS20-LB";

        dataConfiguration.frequencyBand = getFrequencyBand(bytes[3]);

        dataConfiguration.subBand = (bytes[4] === 0xFF) ? "NULL" : bytes[4];

        dataConfiguration.firmwareVersion = (bytes[1] & 0x0F) + '.' +
            ((bytes[2] >> 4) & 0x0F) + '.' +
            (bytes[2] & 0x0F);

        dataLifecycle.batteryVoltage = ((bytes[5] << 8 | bytes[6]) & 0x3FFF) / 1000;
    }
