  ...
  for (let i = 0; i < decoded.data.measurements.length; i++) {
    switch (decoded.data.measurements[i].typeName) {
      case "battery voltage":
        ...
        break;
      case "temperature":
        if (decoded.data.measurements[i].address === "25") {
          data.gasTemperature = decoded.data.measurements[i].value;
        } else if (decoded.data.measurements[i].address === "") {
      ...
      case "relative humidity":
        ...
        break;
      case "pressure":
        data.differentialPressure = decoded.data.measurements[i].value;
        break;
      case "velocity":
        data.velocity = decoded.data.measurements[i].value;
        break;
      default:
        data.status = "Error";
    }
...
function consume(event) {
  ...
  if (data.data.differentialPressure !== undefined) {
    emit("sample", {
      data: { differentialPressure: data.data.differentialPressure },
  ...
  if (data.data.gasTemperature !== undefined) {
    ...
    emit("sample", {
      data: {
        gasTemperature: data.data.gasTemperature,
        gasTemperatureF: data.data.gasTemperatureF,
      },
  ...
  if (data.data.velocity !== undefined) {
    emit("sample", {
      data: { velocity: data.data.velocity },
      topic: "velocity",
    });
    delete data.data.velocity;
  }
}
