...
function payloadV2Parse(bytes, date, utils) {
    ...
    function extractPayloadDateTime(extractPayloadDateTimeBytes, extractPayloadDateTimeDate) {
        return new Date(Number(extractPayloadDateTimeDate) - (utils.unsignedNbrFromBytes(extractPayloadDateTimeBytes.slice(2, 4)) * 1000));
    }
    ...
    function extractMeasurementDateTime(byte, extractMeasurementDateTimeDate) {
        return new Date(Number(extractMeasurementDateTimeDate) + (utils.unsignedNbrFromByte(byte) * 1000));
    }
    ...
    function extractValue(extractValueBytes) {
        ...
        const value = utils.signedNbrFromBytes(extractValueBytes.slice((3 + addressLen), (3 + addressLen + valueLen)));
        const valueExp = (value / (10 ** precision));
        return utils.toFixedNumber(valueExp, precision, 10);
    }

    function extractValueFromRange(extractValueFromRangeBytes, valueFrom, valueTo) {
        ...
        const value = utils.signedNbrFromBytes(extractValueFromRangeBytes.slice((3 + addressLen + valueFrom), (3 + addressLen + valueTo)));
        const valueExp = (value / (10 ** precision));
        return utils.toFixedNumber(valueExp, precision, 10);
    }

    function parseMeasurement(parseMeasurementBytes, parseMeasurementDate) {
        if (parseMeasurementBytes.length < 4) return raiseError("Measurement length must be at least 4 but was: ".concat(parseMeasurementBytes.length));
        const dateTime = extractMeasurementDateTime(parseMeasurementBytes[1], parseMeasurementDate);
        ...
        if (parseMeasurementBytes.length !== expectedLen) return raiseError("Measurement bytes corrupted. Expected length: ".concat(expectedLen, " but was: ", parseMeasurementBytes.length));
        ...
        if (utils.checkIfArrInclude(values4Types, type)) {
            if (valueLen !== 8) return raiseError("Extender measurement bytes corrupted. Expected value length: 8 but was: ".concat(parseMeasurementBytes.length));
            return [
                payloadV2Measurement(`${address}00`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, 0, 2)),
                payloadV2Measurement(`${address}01`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, 2, 4)),
                payloadV2Measurement(`${address}02`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, 4, 6)),
                payloadV2Measurement(`${address}03`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, 6, 8))
            ];
        }
        if (utils.checkIfArrInclude(values3Types, type)) {
            if (valueLen !== 6) return raiseError("Values 3 measurement bytes corrupted. Expected value length: 6 but was: ".concat(parseMeasurementBytes.length));
            return [
                payloadV2Measurement(`${address}00`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, 0, 2)),
                payloadV2Measurement(`${address}01`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, 2, 4)),
                payloadV2Measurement(`${address}02`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, 4, 6))
        ...
        if (utils.checkIfArrInclude(values2Types, type)) {
            ...
            return [
                payloadV2Measurement(`${address}00`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, 0, valueLen / 2)),
                payloadV2Measurement(`${address}01`, dateTime.toISOString(), type, typeName, typeUnits, extractValueFromRange(parseMeasurementBytes, valueLen / 2, valueLen))
            ];
        }
    ...
    function parse(parseBytes, parseDate) {
        const measurements = [];
        if (parseBytes.length < 7) return raiseError("Payload length must be at least 7 but was: ".concat(bytes.length));
        let currPos = 4;
        ...
        while (currPos < parseBytes.length) {
            ...
            measLen = 3 + extractAddressLength(parseBytes[currPos + 2]) + extractValueLength(bytes[currPos + 2]);
            ...
            const currentMeas = parseMeasurement(parseBytes.slice(currPos, currPos + measLen), dateTime);
        ...
        return payload(payloadV2(extractPayloadCnt(parseBytes), dateTime.toISOString(), measurements), [], []);
    }
...
function decoder(port, bytes) {
    ...
    for(let i=0; i<decoded.data.measurements.length; i++) {
        switch(decoded.data.measurements[i].typeName){
            case 'temperature':
                data.temperature = decoded.data.measurements[i].value;
                break;
            case 'relative humidity':
                data.humidity = decoded.data.measurements[i].value;
                break;
            case 'pressure':
                data.pressure = decoded.data.measurements[i].value;
                break;
            case 'illuminance':
                data.illuminance = decoded.data.measurements[i].value;
                break;
            case 'TVOC concentration':
                data.tvoc = decoded.data.measurements[i].value;
                break;
            case 'CO':
                data.co = decoded.data.measurements[i].value;
                break;
            case 'CO2':
                data.co2 = decoded.data.measurements[i].value;
                break;
            case 'sound pressure level':
                data.sound = decoded.data.measurements[i].value;
                break;
            case 'dust concentration':
                switch(decoded.data.measurements[i].address){
                    case '02':
                        data.pm2_5 = decoded.data.measurements[i].value;
                        break;
                    case '03':
                        data.pm4 = decoded.data.measurements[i].value;
                        break;
                    case '04':
                        data.pm10 = decoded.data.measurements[i].value;
                        break;
                    default:
                        data.status = "Error";
                }
...
function consume(event) {
    ...
    if(data.data.pressure !== undefined){
        ...
        delete data.data.pressure;
    }
    if(data.data.illuminance !== undefined){
        emit("sample", {
            data: {illuminance: data.data.illuminance},
            topic: "illuminance",
        });
        delete data.data.illuminance;
    }
    if(data.data.tvoc !== undefined){
        emit("sample", {
            data: {tvoc: data.data.tvoc},
            topic: "tvoc",
        });
        delete data.data.tvoc;
    }
    if(data.data.co !== undefined){
        emit("sample", {
            data: {co: data.data.co},
            topic: "co",
        });
        delete data.data.co;
    }
    if(data.data.co2 !== undefined){
        emit("sample", {
            data: {co2: data.data.co2},
            topic: "co2",
        });
        delete data.data.co2;
    }
    if(data.data.sound !== undefined){
        emit("sample", {
            data: {sound: data.data.sound},
            topic: "sound",
        });
        delete data.data.sound;
    }
    if(data.data.pm2_5 !== undefined){
        emit("sample", {
            data: {pm2_5: data.data.pm2_5},
            topic: "pm2_5",
        });
        delete data.data.pm2_5;
    }
    if(data.data.pm4 !== undefined){
        emit("sample", {
            data: {pm4: data.data.pm4},
            topic: "pm4",
        });
        delete data.data.pm4;
    }
    if(data.data.pm10 !== undefined){
        emit("sample", {
            data: {pm10: data.data.pm10},
            topic: "pm10",
        });
        delete data.data.pm10;
    }
