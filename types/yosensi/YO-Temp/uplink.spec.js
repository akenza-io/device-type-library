...
describe("Yosensi YO Temp uplink", () => {
  ...
  before((done) => {
    utils.loadSchema(`${__dirname}/externalTemperature1.schema.json`).then((parsedSchema) => {
  ...
  before((done) => {
    utils.loadSchema(`${__dirname}/externalTemperature2.schema.json`).then((parsedSchema) => {
  ...
  before((done) => {
    utils.loadSchema(`${__dirname}/externalTemperature3.schema.json`).then((parsedSchema) => {
  ...
  describe("consume()", () => {
    it("should decode the Yosensi YO Temp payload", () => {
      const data = {
        data: {
          port: 0,
          payloadHex: "0200000008000111880d000100f7100000380d00110000ea0d00110100db0d00110200d6"  
        },
      };
      ...
      utils.expectEmits((type, value) => {
        ...
        utils.validateSchema(value.data, externalTemperature1Schema, { throwError: true });
      });
      ...
      utils.expectEmits((type, value) => {
        ...
        utils.validateSchema(value.data, externalTemperature2Schema, { throwError: true });
      });
      ...
      utils.expectEmits((type, value) => {
        ...
        utils.validateSchema(value.data, externalTemperature3Schema, { throwError: true });
      });
