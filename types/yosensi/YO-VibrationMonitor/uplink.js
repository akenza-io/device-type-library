  ...
  for (let i = 0; i < decoded.data.measurements.length; i++) {
    switch (decoded.data.measurements[i].typeName) {
      case "battery voltage":
        ...
        break;
      case "temperature":
        if (decoded.data.measurements[i].address === "4a") {
          data.externalTemperature = decoded.data.measurements[i].value;
...
function consume(event) {
  ...
  if (data.data.externalTemperature !== undefined) {
    data.data.externalTemperatureF = cToF(data.data.externalTemperature);
    emit("sample", {
      data: { externalTemperature: data.data.externalTemperature, externalTemperatureF: data.data.externalTemperatureF },
