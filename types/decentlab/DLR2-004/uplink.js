      ...
      values: [
        {
          ...
          convert: function (x) {
            return (
              ((3 * (x[0] - 32768)) / 32768 / 2 / this.PARAMETERS.R) * 1000
            );
          },
          unit: "mA",
        },
...
function consume(event) {
  ...
  if (event.device !== undefined) {
    if (event.device.customFields !== undefined) {
      ...
      if (customFields.R_shuntResistor !== undefined) {
        decentlab_decoder.PARAMETER.R = customFields.R_shuntResistor;
      }
    }
  }
  ...
  const data = {};
  data.current = sample.current.value;
