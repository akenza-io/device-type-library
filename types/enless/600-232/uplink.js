...
function parseAlarmStatus(bytes) {
  ...
  return {
    temperature_1_high: Boolean(alarmStatus & 0x0001),
    temperature_1_low: Boolean(alarmStatus & 0x0002),
	temperature_2_high: Boolean(alarmStatus & 0x0004),
    temperature_2_low: Boolean(alarmStatus & 0x0008),
  };
}
...
function consume(event) {
  ...
  // Temperature 1 (2 bytes Int16 BE / 10)
  decoded.temperature_1 = readInt16BE(bytes.slice(6, 8)) / 10;

  // Temperature 2 (2 bytes Int16 BE / 10)
  decoded.temperature_2 = readInt16BE(bytes.slice(8, 10)) / 10;
