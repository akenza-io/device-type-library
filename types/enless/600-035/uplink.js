...
function consume(event) {
  ...
  if (bytes.length !== 12) {
    throw new Error(
      "Invalid payload length: " + bytes.length + " bytes, expected 12 bytes.",
    );
  }
  ...
  // --- Current (mA, UInt16 BE / 1000) ---
  decoded.current = readUInt16BE(bytes.slice(6, 8)) / 1000;

  // --- Alarm Status (Little Endian) ---
  const alarmStatus = readUInt16LE(bytes.slice(8, 10));
  alarm.currentHigh = Boolean(alarmStatus & 0x0001);
  alarm.currentLow = Boolean(alarmStatus & 0x0002);

  // --- Status: Battery & Msg Type (Little Endian) ---
  const status = readUInt16LE(bytes.slice(10, 12));
