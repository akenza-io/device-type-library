...
function consume(event) {
  ...
  if (decoded.CONDENSATION_RAW_ONLY !== undefined) { data.condensationRaw = decoded.CONDENSATION_RAW_ONLY; } // Overwrite if present
  ...
  if (decoded.CMD !== undefined) status.controlCommand = decoded.CMD;
  if (decoded.HEARTBEAT !== undefined) { status.heartbeatInterval = decoded.HEARTBEAT; }
  if (decoded.HYSTERESIS !== undefined) status.hysteresis = decoded.HYSTERESIS;
  if (decoded.MEAS_INTERVAL !== undefined) { status.measurementInterval = decoded.MEAS_INTERVAL; }
  if (decoded.BIN_LATENCY !== undefined) { status.digitalInputLatency = decoded.BIN_LATENCY; }
  if (decoded.OCCU_DISABLE_TIME !== undefined) { status.occupancyDisablingTime = decoded.OCCU_DISABLE_TIME; }
  if (decoded.OCCU_FOLLOWUP_TIME !== undefined) { status.occupancyFollowUpTime = decoded.OCCU_FOLLOWUP_TIME; }
  if (decoded.LW_PORT !== undefined) status.lorawanPort = decoded.LW_PORT;
  if (decoded.LW_ADR !== undefined) status.lorawanAdr = decoded.LW_ADR;
  if (decoded.LW_DR !== undefined) status.lorawanDataRate = decoded.LW_DR;
  if (decoded.LW_TX_POWER !== undefined) { status.lorawanTxPower = decoded.LW_TX_POWER; }
  if (decoded.LW_CHANNEL_MASK !== undefined) { status.lorawanChannelMask = decoded.LW_CHANNEL_MASK; }
  if (decoded.LW_NB_TRANS !== undefined) { status.lorawanNbTrans = decoded.LW_NB_TRANS; }
  if (decoded.LW_REJOIN_INTERVAL !== undefined) { status.lorawanRejoinInterval = decoded.LW_REJOIN_INTERVAL; }
  if (decoded.LW_CONFIRMATION !== undefined) { status.lorawanConfirmation = decoded.LW_CONFIRMATION; }
