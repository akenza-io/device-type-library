...
function consume(event) {
  ...
  if (payload[0] !== SETTINGS_HEADER) {
    ...
    data.targetTemperature = res.setpoint;
    ...
    if (deleteUnusedKeys(data)) {
      ...
      if (data.temperature == undefined) {
        // Its not nice but i got to default them if the wrong message is sent first
        if (state.lastTemperature == undefined) {
          state.lastTemperature = 25;
          state.lastHumidity = 40;
        }
        data.temperature = state.lastTemperature;
        data.humidity = state.lastHumidity;
      }

      emit("sample", { data, topic: "default" });

      state.lastTemperature = data.temperature;
      state.lastHumidity = data.humidity;
      emit("state", state);
    }
