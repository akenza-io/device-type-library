function consume(event) {
  ...
  switch (header) {
    ...
    case 2100: {
      ...
      sample.minutesSinceLastOccupancy = 0; // Always give out minutesSinceLastOccupancy for consistancy
      if (sample.occupied) {
        delete state.lastOccupancyTimestamp; // Delete last occupancy timestamp
      } else if (state.lastOccupancyTimestamp !== undefined) {
        sample.minutesSinceLastOccupancy = Math.round((time - state.lastOccupancyTimestamp) / 1000 / 60); // Get free since
      } else if (state.lastOccupancyValue) { //
        state.lastOccupancyTimestamp = time; // Start with first no occupancy
      }

      if (Number.isNaN(sample.minutesSinceLastOccupancy)) {
        sample.minutesSinceLastOccupancy = 0;
      }
